            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <router-link :to="`/asignaturas/add`">
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/asignaturas'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="idasignatura" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="idasignatura" header="Idasignatura" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/asignaturas/view/${data.idasignatura}`">
                                                    {{ data.idasignatura }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="areaconocimiento" header="Areaconocimiento" >
                                            <template #body="{ data, index }">
                                                {{ data.areaconocimiento }}
                                            </template>
                                        </Column>
                                        <Column  field="electivaobligatoria" header="Electivaobligatoria" >
                                            <template #body="{ data, index }">
                                                {{ data.electivaobligatoria }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'asignaturaslist',
		},
		apiPath: {
			type: String,
			default: 'asignaturas/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/asignaturas/view/${data.idasignatura}`,
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			to: `/asignaturas/edit/${data.idasignatura}`,
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.idasignatura) },
			icon: "pi pi-trash"
		}
	]
	}
